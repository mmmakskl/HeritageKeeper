...
function initializeAuthSystem() {
  ...
  const protectedPages = ['my_collections_index.html', 'my_account_index.html', 'in_collection_index.html'];
  const currentPage = window.location.pathname.split('/').pop();
  
  if (protectedPages.includes(currentPage) && !localStorage.getItem('userToken')) {
      const redirect = encodeURIComponent(window.location.pathname);
      window.location.href = `log_in_index.html?redirect=${redirect}`;
      return;
  }

  // If on auth pages and already logged in, redirect to collections
  const authPages = ['log_in_index.html', 'sign_up_index.html'];
  if (authPages.includes(currentPage)) {
      const urlParams = new URLSearchParams(window.location.search);
      const redirect = urlParams.get('redirect') || 'my_collections_index.html';
      ...
      // Auto-fill form fields from URL params if present
      const email = urlParams.get('email');
      const password = urlParams.get('password');
      
      if (email && document.querySelector('input[type="email"]')) {
          document.querySelector('input[type="email"]').value = decodeURIComponent(email);
      }
      
      if (password && document.querySelector('input[type="password"]')) {
          document.querySelector('input[type="password"]').value = decodeURIComponent(password);
      }
  }
